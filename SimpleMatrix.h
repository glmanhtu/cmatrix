...
inline void EuclideanDistanceMatrix(Matrix<double> *X, Matrix<double> *D)
{
  int i, j, k;
  double temp;
  if (D == NULL)
...
inline void ElementCopy(Matrix<double> *X, Matrix<double> *Y)
{
  if (Y == NULL)
  {
    printf("Input matrix pointer is NULL.\n");
...
inline Matrix<double> *MDS_UCF(Matrix<double> *D, Matrix<double> *X0, int dim, int iter)
{
  if (D->rows() != D->columns())
  {
